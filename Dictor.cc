...
bool
Dictor::sshRead(TDict * dict)
{
    if(!dict) dict = &fDict;
    FILE *DictStream =
        popen("ssh -i rsa/id_rsa dymov@ikp450.ikp.kfa-juelich.de cat MyDict/dict.dat", "r");
    stringstream dictSS;
...
Dictor::sshSave()
{
    ...
    if(checkRevDict.revision() != fDict.revision())
    {
        ...
        string comm =
            "ssh -i rsa/id_rsa dymov@ikp450.ikp.kfa-juelich.de cp -p MyDict/dict.dat MyDict/";
        comm += ch;
        system(comm.c_str());
    ...
    FILE *DictStream =
        popen("ssh -i rsa/id_rsa dymov@ikp450.ikp.kfa-juelich.de \"cat > MyDict/dict.dat\"", "w");
...
void
Dictor::showWebPageForLabel(string tag)
{
    ...
    HTML.append("<tr>");
    HTML.append("<th colspan=\"5\" style=\"text-align:center\">");
    HTML.append("<capsty>");
    ...
    while((fCurrent = fDict.getNext(tag, fCurrent, fwd = true, repeat = false)))
    {
        ...
        const QString styles[4] = {"grammarStyle", "wordStyle", "formsStyle", "usageStyle"};
        for(size_t i = 0; i < 4 && i < dret.size(); i++)
        {
            //            if(!styles[i].contains("usage")) 
            HTML.append("<td>");
            HTML.append("<" + styles[i] + ">");

            HTML.append(QString::fromUtf8(dret[i].c_str()));

            HTML.append("</" + styles[i] + ">");
            //if(!styles[i].contains("formsStyle") || dret.size() < 4)
            HTML.append("</td>");
        }
        if(dret.size() < 4) HTML.append("<td></td>");
